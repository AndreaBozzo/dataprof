...
impl ProgressTracker {
    pub(crate) fn new(sink: ProgressSink, interval: Duration) -> Self {
        let now = Instant::now();
        Self {
            sink,
            start_time: now,
    ...
    pub(crate) fn emit_schema(&mut self, column_names: Vec<String>) {
        if self.schema_sent {
            return;
        }
    ...
    pub(crate) fn emit_finished(&mut self, truncated: bool) {

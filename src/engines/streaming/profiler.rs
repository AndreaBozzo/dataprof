...
use crate::core::sampling::{ChunkSize, SamplingStrategy, SampleInfo};
use crate::engines::streaming::progress::{ProgressCallback, ProgressTracker};
use crate::types::{QualityReport, ColumnProfile, FileInfo, ScanInfo};
...
impl StreamingProfiler {
    ...
    pub fn analyze_file(&self, file_path: &Path) -> Result<QualityReport> {
        ...
        let mut all_column_data: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
        ...
        // Process in chunks
        let mut current_chunk_data: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
        for header in headers.iter() {
        ...
        for (row_index, result) in reader.records().enumerate() {
            ...
            if !self.sampling_strategy.should_include(row_index, total_rows_processed + 1) {
                continue;
            }

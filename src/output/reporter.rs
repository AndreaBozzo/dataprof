...
impl TerminalReporter {
        ...
        for (idx, issue) in sorted_issues.iter().enumerate() {
            ...
            match issue {
                ...
                QualityIssue::Outliers {
                    ...
                    println!("[{}]: Outliers detected (>{}σ)", column.yellow(), threshold);
        ...
        for profile in profiles.iter().take(5) {
            ...
            match &profile.stats {
                ColumnStats::Numeric { min, max, mean, .. } => {
                    println!("   └─ Range: [{:.2} - {:.2}], Mean: {:.2}", min, max, mean);
                }
                ColumnStats::Text {
                    min_length,
                    max_length,
    ...
    fn print_recommendations(issues: &[QualityIssue]) {
        ...
        if !fixes.is_empty() {
            println!(
                "   dataprof fix <file> --{} --output cleaned.csv",
                fixes.join(" --")
            );
        }

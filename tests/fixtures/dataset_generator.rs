...
impl DatasetGenerator {
    /// Generate dataset to temporary directory
    pub fn generate_temp(config: &DatasetConfig) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let temp_dir = TempDir::new()?;
        let file_path = temp_dir.path().join(format!("{}.csv", config.name));
        Self::generate_to_path(config, &file_path)?;

        // Return path but keep temp_dir alive
        std::mem::forget(temp_dir);
        Ok(file_path)
    }

    /// Generate dataset to specific path
    pub fn generate_to_path(config: &DatasetConfig, path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn generate_basic(writer: &mut BufWriter<File>, config: &DatasetConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn generate_mixed(writer: &mut BufWriter<File>, config: &DatasetConfig) -> Result<(), Box<dyn std::error::Error>> {
        writeln!(writer, "id,name,email,age,salary,created_at,active,score,category,description")?;
    ...
    fn generate_numeric(writer: &mut BufWriter<File>, config: &DatasetConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn generate_wide(writer: &mut BufWriter<File>, config: &DatasetConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn generate_deep(writer: &mut BufWriter<File>, config: &DatasetConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn generate_unicode(writer: &mut BufWriter<File>, config: &DatasetConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn generate_messy(writer: &mut BufWriter<File>, config: &DatasetConfig) -> Result<(), Box<dyn std::error::Error>> {
        writeln!(writer, "id,name,age,salary,notes,status,created,score")?;

        let rows = config.default_rows();
        for i in 0..rows {
            let name = if i % 20 == 0 { "" } else { &format!("User {}", i) };
            let age = if i % 15 == 0 { "".to_string() } else { (18 + (i % 65)).to_string() };
            let salary = if i % 25 == 0 { "N/A".to_string() } else { format!("{}", 30000 + (i % 100000)) };
            ...
            let created = if i % 30 == 0 { "invalid-date" } else { "2023-01-15" };
            let score = if i % 12 == 0 { "".to_string() } else { format!("{:.1}", (i as f64 * 1.5) % 100.0) };
...
impl StandardDatasets {
    ...
    pub fn micro(pattern: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        let config = DatasetConfig::new(DatasetSize::Micro, pattern);
        DatasetGenerator::generate_temp(&config)
    }

    /// Get small dataset
    pub fn small(pattern: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let pattern = Self::parse_pattern(pattern)?;
        let config = DatasetConfig::new(DatasetSize::Small, pattern);
        DatasetGenerator::generate_temp(&config)
    }
    ...
    pub fn medium(pattern: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let pattern = Self::parse_pattern(pattern)?;
        let config = DatasetConfig::new(DatasetSize::Medium, pattern);
        DatasetGenerator::generate_temp(&config)
    }
    ...
    pub fn large(pattern: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let pattern = Self::parse_pattern(pattern)?;
        let config = DatasetConfig::new(DatasetSize::Large, pattern);
        DatasetGenerator::generate_temp(&config)
    }
...
mod tests {
    ...
    fn test_basic_dataset_generation() {
        ...
        let path = DatasetGenerator::generate_temp(&config).unwrap();

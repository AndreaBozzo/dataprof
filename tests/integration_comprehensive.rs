// DataProfiler v0.3.0 old suite test, kept for regression purposes
...
fn test_streaming_processing() -> Result<()> {
    ...
    assert!(
        report.scan_info.total_columns > 0,
    ...
    println!(
        ...
        report.scan_info.rows_scanned, report.scan_info.total_columns
    );
    Ok(())
}
